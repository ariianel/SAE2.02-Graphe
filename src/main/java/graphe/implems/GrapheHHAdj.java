...
public class GrapheHHAdj implements IGraphe {
    ...
    public void oterSommet(String noeud) {
        hhadj.remove(noeud);
        for (String srcDeArc: hhadj.keySet()) { //B, C, D, etc == les clés
            oterArcPourSommetSansVerif(srcDeArc,noeud);
        }
    }

    /**
     * Supprime l'arc qui relie le sommet source donné au sommet destination donné sans vérifier son existence car pas
     * besoin, cette méthode est seulement utilisée dans une itération des clés de la HashMap(que sur des éléments de la Hashmap)
     * @param srcDeArc le sommet source de l'arc à supprimer.
     * @param noeud le sommet destination de l'arc à supprimer.
     */
    private void oterArcPourSommetSansVerif(String srcDeArc, String noeud) {
        hhadj.get(srcDeArc).remove(noeud);
    }
    ...
    public int getValuation(String src, String dest) {
        ...
        return -1; //si l'arc n'existe pas, retourne -1.
    }
    /**
    ...
    public String toString() {
        return toAString();//Permet de retirer les deux derniers caractères de la chaîne sb
    }
}

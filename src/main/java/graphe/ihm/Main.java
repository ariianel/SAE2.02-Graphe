...
public class Main {
    public static final String GRAPHES_REP = "graphes/full";
    public static final String REPONSES_REP = "reponses/full";
    ...
    private static long chronoTousLesGraphesDeType(IGraphe g) {
        ...
        for (CheminATrouver cat : importer) {
            dureeTotale += chrono(cat.getGraph(), cat.getSD_arc().getNoeudSource(),
                    cat.getSD_arc().getNoeudDest(), cat.getDistance_attendue());
        }
        return dureeTotale;
    }
